	...
	_axios2.default.get("/api/stores/near?lat=" + lat + "&lng=" + lng).then(function (res) {
		...
		var markers = places.map(function (place) {
			var _place$location$coord = _slicedToArray(place.location.coordinates, 2),
			    _lng = _place$location$coord[0],
			    _lat = _place$location$coord[1];

			var position = { lat: _lat, lng: _lng };
...
function makeMap(mapDiv) {
	...
	var _input = (0, _bling.$)(" [name='geolocate'] ");
	var autocomplete = new google.maps.places.Autocomplete(_input);
...
var likeForms = (0, _bling.$$)("form.heart");
